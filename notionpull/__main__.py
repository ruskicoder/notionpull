...
class Scraper:
    ...
    def _expand_toggle_blocks(expanded_toggle_blocks=[]) -> None:
        ...
        for block in toggle_blocks:
            ...
            is_expanded = "(0deg)" in block_style
            if not is_expanded:
    ...
    def _insert_injection_hooks(soup: BeautifulSoup) -> None:
        ...
        for toggle_block in toggle_blocks:
            ...
            if toggle_button and toggle_content:
                toggle_button["class"] = toggle_block.get("class", []) + ["notionpull-toggle-button"]
                toggle_content["class"] = toggle_content.get("class", []) + ["notionpull-toggle-content"]
                toggle_content.attrs["notionpull-toggle-id"] = toggle_button.attrs["notionpull-toggle-id"] = uuid.uuid4()
    ...
    def _link_anchors(soup: BeautifulSoup) -> List[str]:
        ...
        for a in anchors:
            ...
            if is_table_of_contents:
                ...
                url = arr[0]
                a["href"] = f"#{arr[-1]}"
                a["class"] = a.get("class", []) + ["notionpull-anchor-link"]
